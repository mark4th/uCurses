...
// ...except i was benchmarking against GLIBC, whose qsort is a merge sort
// (a clean ~n*log2(n) comparisons, and it borrows a whole scratch buffer
// to get there).  so all those FUDGE_FACTOR timings above are shell vs
// merge, not shell vs shell.  turns out mine does ~2x the comparisons and
// still lands within about 15% on the clock - being in place and cache
// friendly hides most of the algorithmic gap.  uClibc really IS a shell
// sort; glibc is not.  measure the thing you think you are measuring :)
