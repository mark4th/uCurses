...
static uint8_t sinewave(uint8_t i)
{
    return (uint8_t)(128.0 + 127.0 * sin(i * (2.0 * M_PI / 256.0)));
}
...
static void do_win_demo(void)
{
    ...
    uC_win_set_pos(win1, x1, wy1);
    uC_win_set_pos(win2, x2, wy2);

    // add respective increments to x1, wy1, x2, wy2
    x1 += x1i;   wy1 += wy1i;
    x2 += x2i;   wy2 += wy2i;
    ...
    // for example initially the x1 increment is 1 and the wy1
    ...
    if (wy1i == 0)    // if we are moving horizontally...
    {
