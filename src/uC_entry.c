...
static FILE *entry_fp;
static char e_line_buff[64];

// -----------------------------------------------------------------------

static char *next_arg(void)
{
    static char arg_buff[64];
    ...
    while (fread(&c, 1, sizeof(char), entry_fp) == 1)
    {
        if ((i == 63)  || (c == '\0') ||
           (c == '\n') || (c == '\r'))
        {
...
static void print_switch(char *path)
{
    ...
    {
        memset(e_line_buff, 0x20, 63);
        hash = fnv_hash(s);
        snprintf(e_line_buff, 63, "    { 0x%08x, %s", hash, s);

        e_line_buff[strlen(e_line_buff)] = 0x20;
