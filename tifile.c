...
static void map_tifile(void)
{
  ...
  strcpy(&ti_file[0], "/usr/share/terminfo/x/");
  ti_file[20] = term[0];
  strncat(&ti_file[0], term, 128-21);  // magic numbers ftw

  stat(ti_file, &st);
  ti_size = st.st_size;

  fd = open(ti_file, O_RDONLY, 0);
  if(-1 == fd)
  {
  ...
  ti_map = mmap(NULL, ti_size, PROT_READ, MAP_PRIVATE, fd, 0);
...
void uCurses_init(void)
{
  ...
  buffer = malloc(65535);   // allocate 64k for compiled escape sequences
